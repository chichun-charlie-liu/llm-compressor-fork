...
# NOTE kv_cache_scheme and ssm_state_scheme in hybrid models, like Grantie4, can be
...
if isinstance(getattr(recipe, "kv_cache_scheme", None), QuantizationArgs):
    SAVE_DIR += "-FP8-KV"
if isinstance(getattr(recipe, "ssm_state_scheme", None), QuantizationArgs):
    SAVE_DIR += f"-ssm-{recipe.ssm_state_scheme.strategy}"
    if recipe.ssm_state_scheme.strategy == QuantizationStrategy.GROUP:
        SAVE_DIR += f"grp{recipe.ssm_state_scheme.group_size}"
